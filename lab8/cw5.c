...
if ('a'<=litera && litera<='z')  return (int)litera - (int)'a';
  else      if ('A'<=litera && litera<='Z')  return (int)litera - 
(int)'A';
   else  return -1;
}
 char  litera_o_numerze (int n)
{
   ...
   main () {
          ...
          int liczniki_p[DL_ALFABETU], liczniki_k[DL_ALFABETU], i;
         // inicjalizacja licznikow:
       for (i=0; i<DL_ALFABETU; i++)
      {
        liczniki_p[i] = 0;      liczniki_k[i] = 0;
       }
       printf("Podaj tekst: \n");
  ...
  i=numer_litery(zold);
   if(i>=0)
   //W razie kropki na koncu zdania itp
          zolder=zold;
          zold=znak;      znak = getchar();
