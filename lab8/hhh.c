...
int  numer_litery (char litera) {      // duze litery sa sklejane z malymi; nielitery uzyskuja numer -1    
 ...
 char  litera_o_numerze (int n) {      // jesli n<0 lub n>=DL_ALFABETU -- sygnal bledu    
...
for (i=0; i<DL_ALFABETU; i++)        
  for(j=0; j<DL_ALFABETU; j++)        
     liczniki[i][j] = 0;        // wczytywanie tekstu i zliczanie liter: 
  znakold = getchar();    
if(znakold == EOF){    
